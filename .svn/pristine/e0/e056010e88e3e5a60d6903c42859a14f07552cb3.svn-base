...
export default {
  ...
  mounted () {
    // 其他页面跳转
    if (
      this.$store.state.user.analysisObj.panelId &&
      this.$store.state.user.analysisObj.siteId &&
      this.$store.state.user.analysisObj.menuId &&
      this.$store.state.user.analysisObj.curId &&
      this.$store.state.user.analysisObj.skipType
    ) {
      // 请求菜单
      let obj = {}
